...
public record AppearanceSyncS2C(int playerId, CompoundTag nbt) {
    ...
    public static AppearanceSyncS2C decode(FriendlyByteBuf buf) {
        return new AppearanceSyncS2C(buf.readVarInt(), buf.readNbt());
    }

    public static void handle(AppearanceSyncS2C msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> ClientPacketHandler.handleAppearanceSync(msg)));
